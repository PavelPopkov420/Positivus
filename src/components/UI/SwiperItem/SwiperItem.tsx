...
export const SwiperItem: FC<ISwiperItem> = ({ quote, name, jobTitle }) => {
  return (
    <div className={styles.item}>
      <div className={styles.item__bubble}>
        <svg
          width="606"
          height="266"
          viewBox="0 0 606 266"
        ...
        <div>
          <p className={styles.item__qoute}>{quote}</p>
        </div>
      </div>
      <div className={styles.item__info}>
        <h3 className={styles.item__name}>{name}</h3>
        <p className={styles.item__job}>{jobTitle}</p>
      </div>
    </div>
  );
};
